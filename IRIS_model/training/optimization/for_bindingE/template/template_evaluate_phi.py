...
# Modified to handle TM residue safety, dynamic decoy evaluation, and robust phi evaluation
####################################################################################
            ...
            if res1 in res_list_tmonly:
                for res2 in get_neighbors_within_radius(neighbor_list, res1, r_max + 2.0):
                    ...
                    if CPLEX_name == 'CPLEX_NAME':
                        if res2chain == 'PROT_CHAIN':  # user-defined chain in CPLEX
...
def evaluate_phis_over_training_set(training_set_file, phi_list_file_name, decoy_method, max_decoys, tm_only=False, num_processors=1, CPLEXmodeling=False, CPLEX_name='IDK'):
    ################ MODIFICATION HERE ################
    os.makedirs(phis_directory, exist_ok=True)
    ################ MODIFICATION HERE ################
...
def evaluate_phis_for_protein(training_set, phi_list, decoy_method, max_decoys, tm_only=False, CPLEXmodeling=False, CPLEX_name='IDK'):
    ...
    for phi, parameters in phi_list:
        ...
        phis_to_write = phi(res_list_tmonly_native, res_list_entire_native, neighbor_list, parameters, CPLEXmodeling=CPLEXmodeling, CPLEX_name=CPLEX_name)
        output_file.write(str(phis_to_write).strip('[]').replace(',', '') + '\n')
        output_file.close()

        # Determine decoy file dynamically
        if decoy_method == 'CPLEX_randomization':
            decoy_file = os.path.join(decoys_root_directory, f"{decoy_method}/native_Rmodified.decoys")
        else:
            decoy_file = os.path.join(decoys_root_directory, f"{decoy_method}/{protein}.decoys")

        # Read decoys and limit to max_decoys
        decoy_sequences = read_decoy_sequences(decoy_file)
        num_decoys_to_use = min(max_decoys, len(decoy_sequences))
        ...
        for i_decoy, decoy_sequence in enumerate(decoy_sequences):
            if i_decoy >= num_decoys_to_use:
                break
            mutate_whole_sequence(res_list_entire, decoy_sequence)
            phis_to_write = phi(res_list_tmonly, res_list_entire, neighbor_list, parameters, CPLEXmodeling=CPLEXmodeling, CPLEX_name=CPLEX_name)
            output_file.write(str(phis_to_write).strip('[]').replace(',', ' ') + '\n')
        output_file.close()
...
evaluate_phis_over_training_set(
    "proteins_list_forphi.txt",
    "phi1_list.txt",
    decoy_method='CPLEX_randomization',
    max_decoys=1000000,   # Upper limit; actual number used depends on decoy file length
    tm_only=False,
    num_processors=1,
    CPLEXmodeling=True,
    CPLEX_name='CPLEX_NAME'  # Placeholder to avoid breaking pipeline
)