...
def generate_decoy_sequences(proteins_list_file_name, methods=['RNA_randomization'], num_decoys=[1000], randomSeed=None):
...
def generate_decoy_sequence(protein, method='RNA_randomization', degree=None):
    ...
    if method == 'shuffle':
        ...
        resids_toberandomized = open(
            "randomize_position_RNA.txt", 'r').readline().split(' ')
...
generate_decoy_sequences("proteins_list.txt", methods=['RNA_randomization'], num_decoys=[1000], randomSeed=0)
